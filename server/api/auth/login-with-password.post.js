...
import { checkPassword } from "~/server/utils/hash";
...
async function handleUserValidation(email, password) {
  ...
  if (!user.hashedPassword && user.emailVerified) {
    const connectedOauthAccount = await userActions.findOauthAccountByUserId(
      user.id,
    );
  ...
  const isPasswordCorrect = await checkPassword(user.hashedPassword, password);
